...
// 압축된 데이터 분석 (gzip 헤더 확인)
console.log('압축된 데이터 (처음 20바이트):', compressedArray.slice(0, 20));
console.log('Gzip 매직 넘버 확인:', compressedArray[0], compressedArray[1]);

// 압축된 데이터를 16진수로 출력
const hexString = Array.from(compressedArray.slice(0, 50)).map(b => b.toString(16).padStart(2, '0')).join(' ');
console.log('압축된 데이터 (16진수, 처음 50바이트):', hexString);

// 압축된 데이터의 전체 크기와 구조 분석
console.log('전체 압축 데이터 크기:', compressedArray.length);
console.log('압축 데이터 마지막 10바이트:', compressedArray.slice(-10));
...
if (typeof pako !== 'undefined') {
  try {
    ...
    console.log('=== 파티 데이터 복호화 완료 (pako 사용) ===');
    ...
    console.error('pako로 압축 해제 실패:', error.message);
  }
} else {
  console.log('pako가 로드되지 않았습니다. 브라우저 콘솔에서 실행해주세요.');
}
